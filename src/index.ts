...
import { type CSSProperties, type VNode, h, defineComponent } from 'vue';
...
function getInnerText(input: VNode | VNode[]) {
  let text = '';
  if (input && Array.isArray(input)) {
    text = input[0].children as string;
  } else {
    text = input.children as string;
  }
  return text;
}
 ...
 */
function createStyle(bundle: AnserJsonEntry): CSSProperties {
...
/**
 * Converts an Anser bundle into a vue Node.
...
function convertBundleIntoVue(
  ...
  const content: Array<VNode | string> = [];
...
const Ansi = defineComponent({
  name: 'Ansi',
  props: {
    /** whether links should be converting into clickable anchor tags. */
    linkify: {
      type: Boolean,
      required: false,
      default: undefined,
    },
    /** should render the span with a class instead of style */
    useClasses: {
      type: Boolean,
      required: false,
      default: undefined,
  ...
  setup(props, { attrs, slots }) {
    return () => h(
      ...
      { class: attrs?.class || '' },
      // @ts-ignore
      ansiToJSON(slots?.default() ?? '', props.useClasses ?? false).map(
        convertBundleIntoVue.bind(null, props.linkify ?? false, props.useClasses ?? false),
      ),
    );
  },
});
